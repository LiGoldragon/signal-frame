...
- Schema-emitted `ExtendedHeader` projections — prefix-preserving
  256-byte header records generated by `emit_schema!` from the same
  route table as `ShortHeader`. Byte 0..8 is always the ordinary
  `ShortHeader`, so old triage can still route from the prefix while
  newer readers inspect the longer schema-defined header surface.
...
**Crystallized architecture slice:** per `reports/designer/341-schema-crystallizes-architecture-2026-05-25.md`, schema emission now starts to cover the two-language component model, not only the external wire surface. The first generated effect side is deliberately mechanical: each assembled route emits an internal `Effect` vocabulary, an `EffectTable` that maps `Operation -> Effect`, an `Interact` / `InteractionActor` trait contact point, and closed fan-out output records. Later schema syntax can replace the default one-to-one route mapping with an explicit effect table without changing the runtime shape.
